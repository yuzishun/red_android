...
public class ContactOperateNotifyListenerImpl implements IYWContactOperateNotifyListener {
    ...
    public void onVerifyAddRequest(IYWContact contact, String message) {
        Notification.showToastMsg(MyApplication.application, contact.getUserId()+"用户请求加你为好友");
...
//                 }
    ...
    public void onAcceptVerifyRequest(IYWContact contact) {
        Notification.showToastMsg(MyApplication.application,contact.getUserId()+"用户接受了你的好友请求");
    }
    /**
     * 用户拒绝了你的好友请求
     * todo 该回调在UI线程回调 ，请勿做太重的操作
    ...
    public void onDenyVerifyRequest(IYWContact contact) {
        Notification.showToastMsg(MyApplication.application,contact.getUserId()+"用户拒绝了你的好友请求");
    }
    ...
    public void onSyncAddOKNotify(IYWContact contact) {
        Notification.showToastMsg(MyApplication.application,"云旺服务端（或其它终端）进行了好友添加操作对"+contact.getUserId());
    ...
    public void onDeleteOKNotify(IYWContact contact) {
        Notification.showToastMsg(MyApplication.application,contact.getUserId()+"用户从好友名单删除了您");
    }

    @Override
    public void onNotifyAddOK(IYWContact contact) {
        Notification.showToastMsg(MyApplication.application,"用户添加您为好友了"+contact.getUserId());
