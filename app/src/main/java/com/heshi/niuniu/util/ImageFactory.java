...
/**
 * Created by tianqi on 2017/5/20.
 */
...
public class ImageFactory {
     ...
     * @return
     */
    public static Bitmap getBitmap(String imgPath, int radio) {
...
//        return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
     ...
     * @return
     */
    public static Bitmap ratio(Bitmap image, float pixelW, float pixelH, int max) {
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        //image.compress(Bitmap.CompressFormat.JPEG, 100, os);
        if (os.toByteArray().length / 1024 > max) {//判断如果图片大于1M,进行压缩避免在生成图片（BitmapFactory
            // .decodeStream）时溢出
            os.reset();//重置baos即清空baos
            image.compress(Bitmap.CompressFormat.JPEG, 50, os);//这里压缩50%，把压缩后的数据存放到baos中
        }
//        int options=100;
//        while (os.toByteArray().length / 1024 > max) {
//            // Clean up os
//            os.reset();
//            // interval 10
//            options -= 1;
...
//        }
    ...
    public static void compressAndGenImage(Bitmap image, String outPath, int maxSize) throws
        ...
        while (os.toByteArray().length / 1024 > maxSize) {
         //   ToashUtils.show(context, os.toByteArray().length, Toast.LENGTH_SHORT);
            // Clean up os
            os.reset();
            // interval 10
            options -= 10;
            ...
            image.compress(Bitmap.CompressFormat.JPEG, options, os);
            Log.e("srcNew", os.toByteArray().length/1024 + "");
